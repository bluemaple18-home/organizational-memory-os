...
status: SUPERSEDED_BY_CORRELATION_CLOSEOUT
type: implementation
tier: T1
...
`task_started`／`task_complete` 都已透過 `native_correlation_ref`
（值為 `turn_id`）機制重新映射，見
`.work/CARD-NATIVE-ADAPTERS-CORRELATION-CLOSEOUT-20260914.md`（實作
Owner 簽定的 FP-1-A/FP-2-A/FP-3-A）。這張卡不再是這兩個事件的待辦。
如果未來又找到別的候選事件要映射，開新卡，不要重新打開這張。
